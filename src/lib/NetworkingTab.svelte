...
>
    {#if $connectionState == "CONNECTED"}
        <span
            class="ml-auto {$exitNode ? 'text-green-500' : 'text-amber-500'}"
            title={$exitNode ? "Ready" : "No exit node"}>●</span
        >
    {/if}
</PanelButton>
<p class="mt-4">You can connect to the Internet via Tailscale</p>
<p class="mt-4">
    Using Tailscale is required since browsers do not support TCP/UDP sockets
    (yet!)
</p>
